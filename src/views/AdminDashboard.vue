...
import { getUsuarios, saveUsuarios, getBarbearias, getAgendamentos, saveAgendamentos, getHorarios, isBarbeiroDisponivel } from '@/utils/storage';
...
function cadastrarBarbeiro() {
    ...
    const emailExiste = usuarios.some(u => u.email.toLowerCase() === formBarbeiro.email.toLowerCase());
    if (emailExiste) {
    ...
    alert(`Barbeiro ${novoBarbeiro.nome} cadastrado com sucesso!`);
...
function carregarHorariosReagenda(ag) {
    ...
    horariosDisponiveisReagenda.value = listagemHorariosFixos.value.filter(hora => {
        // Verifica disponibilidade ignorando o próprio agendamento atual (para manter o mesmo horário/dia se quiser)
        return isBarbeiroDisponivel(ag.barbeiroId, formReagenda.data, hora, ag.id);
    });
}
...
function salvarReagendamento(id) {
    ...
    if (idx !== -1) {
        ...
        // Valida disponibilidade final antes de salvar
        const disponivel = isBarbeiroDisponivel(ag.barbeiroId, formReagenda.data, formReagenda.horario, ag.id);
        if (!disponivel) {
            alert("Erro: O barbeiro já possui um compromisso neste novo horário. Escolha outro.");
            return;
        }

        todos[idx].data = formReagenda.data;
        todos[idx].horario = formReagenda.horario;
        todos[idx].status = 'Agendado'; // Caso estivesse cancelado, reativa como agendado ao mudar data/hora
